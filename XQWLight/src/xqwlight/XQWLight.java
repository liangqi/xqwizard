...
public class XQWLight extends MIDlet {
	public static final int DISP_NULL = 0;
	public static final int DISP_STARTUP = 1;
	public static final int DISP_MAINFORM = 2;
    ...
    public void destroyApp(boolean unc) {
		try {
			RecordStore rs = RecordStore.openRecordStore("XQWLight", true);
			rsData[0] = (byte) (disp == DISP_MAINFORM ? level + 1 : 0);
			RecordEnumeration re = rs.enumerateRecords(null, null, false);
			if (re.hasNextElement()) {
				int recordId = re.nextRecordId();
				rs.setRecord(recordId, rsData, 0, RS_DATA_LEN);
